...
class NewsletterDetector:
    ...
    def is_newsletter(self, message: Dict) -> Tuple[bool, Optional[str], List[str]]:
        ...
        newsletter_keywords = ['unsubscribe', 'view in browser', 'preference center', 
        ...
        is_newsletter = (
            len(reasons) >= 2 or 
            'has_list_unsubscribe_header' in reasons or
            any('from_platform:' in r for r in reasons)
        )
    ...
    def analyze_sender_frequency(self, messages: List[Dict]) -> Dict[str, Dict]:
        ...
        for email, stats in sender_stats.items():
            if len(stats['dates']) >= 2:
                dates = sorted([datetime.fromisoformat(d.replace('Z', '+00:00')) 
