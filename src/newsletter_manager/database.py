...
class Database:
    ...
    def _create_tables(self) -> None:
        ...
        # Create indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_newsletters_sender ON newsletters(sender_email)"
        )
        cursor.execute(
    ...
    def upsert_newsletter(
        ...
        # Check if exists
        cursor.execute(
            "SELECT id, total_count FROM newsletters WHERE sender_email = ?", (sender_email,)
        )
        row = cursor.fetchone()
    ...
    def get_all_newsletters(self, category: Optional[str] = None) -> List[Dict]:
        ...
        if category:
            cursor.execute(
                "SELECT * FROM newsletters WHERE category = ? ORDER BY last_seen DESC", (category,)
            )
        else:
